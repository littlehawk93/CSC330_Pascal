...
public class compiler
{
		...
		static private final int NORW = 28;
		...
		{
			LIT(0), OPR(1), LOD(2), STO(3), CAL(4), INT(5), JMP(6), JPC(7), CTS(8), STI(9), LDI(10), LDA(11);
		...
		static String mneumonic[] = new String[23];
		...
		public static String ErrMsg[] =
		{
			...
			"Main block can not have parameters",/* 36 */
			"Ref or val parameter expected",/* 37 */
			"Right parenthesis expected",/* 38 */
			"Only identifiers can be passed by reference"/* 39 */
		};
			...
			if(sym == SYMBOL.LPAREN || sym == SYMBOL.SEMICOLON)
			{
				...
				if(lev > 0)
				{
					if(sym == SYMBOL.LPAREN)
					{
						...
						{
							GetSym();
							if((sym == SYMBOL.REFSYM) || (sym == SYMBOL.VALSYM))
							{
								boolean isRef = sym == SYMBOL.REFSYM;
								do
								{
									GetSym();
									if (sym == SYMBOL.IDENT)
									{
										int[] txx = new int[1];
										txx[0] = tx;
										dx = Enter((isRef) ? OBJECTS.Reference : OBJECTS.Value, lev, dx, txx);
										table[tx].params[dx - 4] = (isRef) ? 1 : 0;
										tx = txx[0];
										GetSym();
									}
									else
				...
				if (sym == SYMBOL.IDENT)
				{
					...
					if(sym == SYMBOL.LPAREN)
					{
						...
						do
						{
							...
							// If reference
							if(table[i].params[p] == 1)
							{
								if(sym != SYMBOL.IDENT)
									Error(39);
								int tI;
								if ((tI = Position(id, tx)) == FALSE)
									Error(11);
								if(table[tI].kind == OBJECTS.Reference)
									Gen(FUNCTIONS.LOD, lev - table[tI].level, table[tI].adr);
								else
									Gen(FUNCTIONS.LDA, lev - table[tI].level, table[tI].adr);
								GetSym();
							}
							else
							{
				...
				case IDENT:
					...
					else
					{
						if(table[i].kind == OBJECTS.Function)
						{
							...
							// MODIFIED CODE BEGIN
							if (table[i].kind == OBJECTS.Variable || table[i].kind == OBJECTS.Reference || table[i].kind == OBJECTS.Value)
							{
								if(table[i].kind == OBJECTS.Reference)
									REFERENCE_FLAG = true;
							}
							else
								Error(12);
